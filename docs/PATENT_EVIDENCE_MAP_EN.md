...
| [0056]–[0058] | Fixed channel tau during inference | model and loader | general CPU/GPU inference |
...
All five values in a table row come from one checkpoint selected by standard-test accuracy. Decreasing and reverse exactly share the nominal budget; uniform is approximate; fixed has zero budget. The paired replay uses the first 12 MNIST test digits from each class under five controlled conditions.
